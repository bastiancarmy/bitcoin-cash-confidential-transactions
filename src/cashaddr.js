...
export function cashHrpExpand(hrp) {
  const expanded = [];
  for (const ch of hrp) {
...
export function cashCreateChecksum(hrp, data) {
  const values = [...cashHrpExpand(hrp), ...data];
  // Append 8 zero "template" words, then run polymod
  const mod = polymod([...values, ...new Array(8).fill(0)]);
  const checksum = [];
  for (let i = 0; i < 8; i++) {
...
export function cashVerifyChecksum(hrp, data) {
  // Full data (payload + checksum) must produce polymod(...) === 0
  return polymod([...cashHrpExpand(hrp), ...data]) === 0n;
}
...
export function convertbits(data, frombits, tobits, pad = true) {
  ...
  for (let value of data) {
    ...
    acc = ((acc << BigInt(frombits)) | value) & maxAcc;
    bits += frombits;
    while (bits >= tobits) {
      bits -= tobits;
  ...
  if (pad) {
    ...
    bits >= frombits ||
    ((acc << BigInt(tobits - bits)) & maxv) !== 0n
  ) {
 ...
 */
function normalizeTypeBits(type) {
 ...
 */
function sizeBitsForHashLen(len) {
  switch (len) {
    case 20: return 0;
    case 24: return 1;
    case 28: return 2;
 ...
 *
 * @param {string} prefix  e.g. "bitcoincash", "bchtest"
 * @param {string|number} type "P2PKH" | "P2SH" or numeric type bits
 * @param {Uint8Array}    payload hash160 / script hash bytes
 */
export function encodeCashAddr(prefix, type, payload) {
 ...
 */
export function decodeCashAddress(addr) {
  ...
  const sizeTable = [20, 24, 28, 32, 40, 48, 56, 64];
