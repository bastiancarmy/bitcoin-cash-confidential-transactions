...
/* ========================================================================== */
/* Raw tx builder + size estimator                                            */
/* ========================================================================== */

/**
 * Build raw tx hex from a tx object:
 * {
 *   version, inputs: [{txid, vout, scriptSig, sequence}], outputs: [{value, scriptPubKey}], locktime
 * }
 *
 * `tx.outputs` may be:
 *  - Array of { value, scriptPubKey } (preferred)
 *  - Uint8Array blob of concatenated outputs (without count)
 *  - Any of the above + explicit `outputsCount` to override count inference
 */
export function buildRawTx(tx) {
  // ---- inputs ----
  const inputs = tx.inputs.map((inp) => {
    const scriptSig =
      inp.scriptSig instanceof Uint8Array ? inp.scriptSig : new Uint8Array();
    return concat(
    ...
    if (typeof outputsCount === 'number') {
      ...
      try {
        ...
        try {
          ...
          console.warn('buildRawTx: could not infer outputs count; assuming 1');
          outputsCount = 1;
          outputsBytes = blob;
        }
      }
      ...
      if (Array.isArray(outItem) && outItem.length === 2) {
        ...
        v = normalizeSats(
          'value' in outItem
            ...
            ? outItem.val
            : (() => {
                throw new Error('buildRawTx: output.value missing');
              })()
        );
        spk = scriptToBytesLoose(outItem.scriptPubKey ?? outItem);
      } else {
        throw new Error('buildRawTx: unsupported output item type');
      }
